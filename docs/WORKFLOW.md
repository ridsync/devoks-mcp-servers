...
### 16. 남용 방어 — "노출"과 "취약"을 구분한 뒤 남은 진짜 문제

저장소 공개 범위를 점검하다 방향이 바뀌었다. 공개된 값들(계정 ID·ARN·엔드포인트·
조직명)을 공격 경로 기준으로 하나씩 따져보니 **어느 것도 보안 통제가 아니었다** —
특히 OIDC 신뢰 정책이 **불변 저장소 ID**로 고정돼 있어 ARN을 알아도 얻는 게 0이다.
"private은 CI 비용 때문에 부담"이라던 근거도 GitHub이 arm64 러너를 private
프리티어 대상으로 바꾸면서 폐기됐다(실사용 월 151분 = 프리티어의 7.5%).

→ **PUBLIC 유지로 확정.**

대신 점검 과정에서 진짜 구멍이 나왔다: **남용 방어가 전무**했다. 무단 요청도
401을 내기 *전에* Lambda가 호출되므로 과금되는데, 스로틀링·예약 동시성·예산
알림이 모두 없어 상한이 없었다(1억 요청 = $175, 10억 = $1,754).

순서가 중요했다 — **Function URL 삭제가 먼저**다. 그것이 API Gateway를
**우회**하므로, 우회로를 두고 API GW만 조이면 나머지가 전부 무의미하다.

```
① Function URL 삭제 + Principal:"*" 권한 회수 → 진입점 3→2, Principal:* 0개
② API Gateway 스로틀링 rate 10/s · burst 20    → 429는 Lambda 미호출 = 최저 비용 거절
③ Lambda 예약 동시성 10 (백스톱)
④ 예산 $10 + FORECASTED 알림
```

③은 처음 5로 잡았다가 **테스트가 정정**했다: 60요청 동시 발사에서 429가 아니라
**503이 48건** 나왔는데, rate limit이 아니라 동시성 5가 먼저 소진된 것이었다.
콜드스타트 1.9초가 5개 겹치면 정상 사용도 막히므로 10으로 올렸다.

> **교훈**: "정보가 공개됨"과 "공격 가능함"은 다르다. 전자를 쫓다가 후자를
> 놓칠 뻔했다. 공개 항목마다 *실제 공격 경로*를 그려보는 게 목록을 세는 것보다
> 빨랐다.

## 현재 상태

**`https://mcp.devoks.kr/mcp`** — Lambda(arm64/512MB, 예약 동시성 10) ←
API Gateway HTTP API(rate 10/s · burst 20) ← ACM(자동 갱신). Function URL은
삭제됐고 Lambda 리소스 정책에 `Principal:"*"`가 없다. 예산 $10 알림 가동.
저장소는 **PUBLIC 유지**(FRD §10 근거). 월 비용 약 **$0.51**(ECR 저장 $0.01 + Route 53 미사용,
