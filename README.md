...
| Knowledge — GitHub 쓰기(이슈 생성·코드 수정 PR) | ❌ 미구현 — 로드맵: [docs/roadmap.md](docs/roadmap.md) |
...
# 필수 키 전체 목록과 함정은 docs/management-guide.md "환경변수" 절 참고.
...
| 가이드 | 내용 |
|---|---|
| [docs/management-guide.md](docs/management-guide.md) | 로컬 실행, 환경변수, MCP 클라이언트 등록, 인증/인가, GitHub 조회 툴, 감사 로그, 컨테이너 빌드, 알려진 제약 |
| [docs/slackbot-guide.md](docs/slackbot-guide.md) | 처리 흐름(handler/worker), 멱등성, 배포 인프라 |
...
| management 로컬 서버 기동 | [docs/management-guide.md#빠른-시작](docs/management-guide.md#빠른-시작) |
| 컨테이너 로컬 빌드 | [docs/management-guide.md#컨테이너-빌드](docs/management-guide.md#컨테이너-빌드) |
...
- [`docs/WORKFLOW.md`](docs/WORKFLOW.md) — 전체 작업 흐름(왜 이 순서, 무엇이 실측으로 뒤집혔는지)
- [`docs/roadmap.md`](docs/roadmap.md) — 확장 예정 스펙(GitHub 쓰기 권한 등 아직 FRD로 확정되지 않은 아이디어)
