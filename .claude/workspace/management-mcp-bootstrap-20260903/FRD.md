...
- **미결 결정:**
  ...
  - **정적 Bearer → OAuth 2.1 전환 — 조건부 트리거(2026-09-14 재정의).** "언젠가 해야 할 일"로 두면 필요 없을 때 하거나 필요한 순간을 놓친다. 아래 중 **하나라도** 해당하면 착수한다:
    1. **클라이언트가 10개를 넘을 때** — `EDGE-021`(Lambda 환경변수 4 KB)이 하드 한계다. 실측: 현재 2,241 B / 4,096 B, 토큰 1개당 약 123 B → **총 16개가 상한**이고 PEM 하나가 예산의 41%(1,674 B)를 먹는다. 10개를 여유 확보선으로 둔다.
       → **먼저 시도할 것은 OAuth가 아니라 SSM 직접 읽기다**(`TASK-056`의 잔여 결정). 원인이 인증 방식이 아니라 4 KB 제약이기 때문이다.
    2. **한 주체가 여러 사람을 대행하기 시작할 때** — Stage 3 Slackbot이 토큰 하나로 여러 Slack 사용자를 대행하면 감사가 전부 `client_id=slackbot`이 되어 "누가 무엇을 읽었는지"가 사라진다.
       → **먼저 시도할 것은 사람별 정적 토큰이다.** `CTR-002`가 이미 `{token: {client_id, role, scopes[]}}` 구조라 사람마다 한 줄씩 발급하면 **코드 변경 0줄로** per-person 감사가 된다.
    3. **퇴사·오프보딩에서 중앙 취소가 요구될 때** — 현재는 환경변수에서 해당 줄을 지우고 설정을 갱신한다(사용자 1~3명이면 1분).
    - OAuth만 주는 것: **토큰 만료 · SSO 연동 즉시 취소 · MFA · 장기 비밀 부재**. 팀이 커지고 입퇴사가 생기는 시점에 의미를 갖고, 그 전에는 비용만이다.
    - `DSN-001`이 교체 지점을 `verifier.py` **한 파일**로 국소화해 둬서 나중에 필요해져도 전환 비용이 작다 — **그것이 지금 하지 않아도 되는 이유이기도 하다.**
