...
- [x] `TASK-050` **[베이스 이미지 취약점]** ECR `scanOnPush`가 CRITICAL 6 / HIGH 11을 보고한다. **전부 `python:3.14-slim-trixie` 베이스의 Debian OS 패키지**이며 우리 Python 코드·의존성은 0건이다(OSV 40패키지 확인). `perl`이 21건 중 13건·CRITICAL 6건 중 5건을 차지하는데 **우리 서버는 perl을 호출하지 않는다**. Debian 보안 트래커 확인 결과 해당 CVE 전부 trixie에서 `status=open`·`fixed=-` — **`apt-get upgrade`로는 한 건도 줄지 않는다**(빌드 시간·레이어만 증가). 실효 있는 선택지: ① 미사용 패키지(`perl` 등) 제거 — CI 스모크가 검증 harness가 되므로 깨지면 즉시 드러난다 ② distroless/alpine 등 베이스 계열 전환 — musl·uv 조합 검증 필요. **현 위험 평가**: 이 CVE들은 컨테이너 안에서 이미 코드 실행이 가능한 상태를 전제하므로, 그 시점엔 CVE가 주 문제가 아니다. 배포를 막지 않되 공개 서비스로 굳히기 전에 ①을 시도할 가치가 있다 — file: `servers/management/Dockerfile`
  ...
  - **측정 결과 — 위 정정이 다시 틀렸다.** `apt-get upgrade`를 넣고 실제 이미지를 스캔한 결과 **CRITICAL 6 / HIGH 11 그대로, 패키지별 건수도 전부 동일**했다. 빌드 로그가 이유를 말한다: `Get:3 ... debian-security trixie-security InRelease` → `0 upgraded, 0 newly installed, 0 to remove and 0 not upgraded.` **`trixie-security`가 이미 소스에 포함돼 있고 apt가 할 일이 없었다.** 즉 최초 결론("apt로는 한 건도 줄지 않는다")이 옳았고, OSV 기반 정정이 틀렸다 — sqlite3의 `+deb13u1`은 **이미 설치돼 있고** ECR이 소스 패키지 버전(`3.46.1-7`)으로 매칭해 보고하는 **오탐**이다.
  - **채택한 조치**: `apt-get upgrade`는 **유지**한다. `0 upgraded`는 베이스 이미지가 *지금* 최신이라는 뜻이지 항상 그렇다는 뜻이 아니다 — 업스트림 보안 업로드와 다음 베이스 재빌드 사이의 창을 캐시된 레이어 1개 비용으로 닫는다. **위 카운트에 대한 해결책이 아니며 그렇게 오해해선 안 된다.**
  - **이 항목의 교훈**: 세 단계 중 두 단계가 틀렸고 측정이 결론을 냈다. 취약점 스캐너의 버전 매칭은 소스/바이너리 패키지 리비전을 구분하지 않을 수 있으므로, "수정판이 있다"는 조회 결과만으로 판단하지 말고 **적용 후 스캔을 다시 돌려 비교**해야 한다.
...
- [x] `TASK-060` **[Medium]** 커스텀 도메인 `mcp.devoks.kr` — **API Gateway HTTP API**(CloudFront 아님, 근거 FRD §7·§10 Step 7). ACM 인증서(`ap-northeast-2`) + HTTP API + Lambda 프록시 통합 + 커스텀 도메인 + 가비아 CNAME 2건(ACM 검증 underscore, `mcp` → 리전 엔드포인트). 완료 후 `MCP_PUBLIC_URL`·`MCP_ALLOWED_HOSTS` 전환 — file: `infra/04-custom-domain.sh` / traces: `CTR-001`, `EDGE-018`, `EDGE-019`
  - **완료(2026-09-10). `https://mcp.devoks.kr/mcp` 라이브.**
    - DNS: 가비아 NS 위임 확인, ACM 검증 CNAME + `mcp` CNAME 2건 모두 해석. 인증서 `ISSUED`(TLS 1.3, `CN=mcp.devoks.kr`, Amazon RSA 2048 M04, 만료 2027-03-26, 자동 갱신). 커스텀 도메인 `AVAILABLE`(REGIONAL, TLS_1_2), API 매핑 `hgw6x7` → `jd2r2h5esk` `$default`.
    - **`AC-002-4` 도메인 기준 재검증**: well-known `resource` = `https://mcp.devoks.kr/mcp` 정확 일치, `authorization_servers` = `['https://mcp.devoks.kr']`. 무인증 `POST /mcp` → **401(421 아님)** = `Host`가 그대로 도착해 `MCP_ALLOWED_HOSTS` 검증을 통과한 증거.
    - 4툴 실 GitHub 호출 전부 성공(374~973 ms), 보안 경계 5종(트래버설 2·allowlist 밖·주입 2) 전부 차단, 정상 경로 과잉 차단 없음.
    - `MCP_PUBLIC_URL`·`MCP_ISSUER_URL`을 도메인으로 전환했다. **도메인이 실제로 응답하는 것을 확인한 뒤에** 바꿨다 — 먼저 바꾸면 well-known의 `resource`가 도달 불가한 URL을 광고해 `AC-002-4` 위반이 된다. `MCP_ALLOWED_HOSTS`는 세 진입점(Function URL·API Gateway 기본·도메인)을 모두 유지해 클라이언트 재설정 전 다운타임이 없다.
  - **ACM 검증이 지연될 때 진단한 방법** — 무작정 기다리지 않고 원인을 좁혔다: CAA 레코드 없음(제약 없음), 기대값 대 실제값 문자 단위 일치, `FailureReason` 없음 → ACM 폴링 백오프로 판단. 확인차 **검증 CNAME이 같은 도메인·계정에서 결정적인지 실측**했고(동일함 확인) 그 사이 원래 인증서도 발급돼 재시도용 인증서는 삭제했다. 트레일링 점(`...amazonaws.com.`)은 FQDN 표기로 정상 동작함을 `dig`로 확인했다.
  - **⚠️ 발견 — 검증이 구버전 이미지를 시험하고 있었다.** 도메인 검증 직후 감사 로그에 `TASK-049`의 신규 `reason_code`가 보이지 않았다. 원인은 함수가 `e9973e1f`(SSM 커밋) 이미지로 돌고 있었고 **CI 배포 스텝은 기본 브랜치 전용**이라 `TASK-043~050`의 코드 변경이 프로덕션에 없었던 것. `TASK-061`과 동일한 명령으로 HEAD 이미지를 배포한 뒤 재검증했다.
    - 새 이미지 첫 호출 **3,647 ms**, 두 번째 **59 ms** — `EDGE-016`의 "새 이미지 첫 1회는 일회성 최적화 비용" 패턴 재확인(이전 8,511 ms보다 작은 것은 이미지 레이어 대부분이 캐시된 갱신이기 때문).
    - **`TASK-049` 프로덕션 확인**: 같은 로그에 전/후가 나란히 남았다 — 배포 전 `error`/`ToolError`, 배포 후 `denied`/`path_traversal_attempt`·`query_qualifier_injection`. 클라이언트 응답 문자열은 양쪽 동일해 `AC-003-5`가 유지됨도 확인했다.
  - (이력) 착수 시점 기록: AWS 측은 완료, DNS 작업에 블록됨. 만든 것: ACM 인증서 `arn:...certificate/64892b2a-e986-4b7d-9154-6f3fb8b797be`(`PENDING_VALIDATION`), HTTP API `jd2r2h5esk`(Lambda 프록시 통합, `$default` 라우트·스테이지, AutoDeploy), API Gateway용 Lambda 호출 권한, `MCP_ALLOWED_HOSTS`에 세 진입점 등록. 재현 스크립트 `infra/04-custom-domain.sh`.
